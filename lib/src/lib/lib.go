...
func (cb RustBuffer) Free() {
	rustCall(func(status *C.RustCallStatus) bool {
		C.ffi_lib_rustbuffer_free(cb, status)
		return false
	})
}
...
func bytesToRustBuffer(b []byte) RustBuffer {
	...
	return rustCall(func(status *C.RustCallStatus) RustBuffer {
		return C.ffi_lib_rustbuffer_from_bytes(foreign, status)
	})
}
...
func uniffiCheckChecksums() {
	...
	scaffoldingContractVersion := rustCall(func(uniffiStatus *C.RustCallStatus) C.uint32_t {
		return C.ffi_lib_uniffi_contract_version(uniffiStatus)
	})
	if bindingsContractVersion != int(scaffoldingContractVersion) {
		// If this happens try cleaning and rebuilding your project
		panic("lib: UniFFI contract version mismatch")
	}
	{
		checksum := rustCall(func(uniffiStatus *C.RustCallStatus) C.uint16_t {
			return C.uniffi_lib_checksum_func_add(uniffiStatus)
		})
		if checksum != 9906 {
			// If this happens try cleaning and rebuilding your project
			panic("lib: uniffi_lib_checksum_func_add: UniFFI API checksum mismatch")
		}
	}
}
...
func Add(a int64, b int64) int64 {
	return FfiConverterInt64INSTANCE.Lift(rustCall(func(_uniffiStatus *C.RustCallStatus) C.int64_t {
		return C.uniffi_lib_fn_func_add(FfiConverterInt64INSTANCE.Lower(a), FfiConverterInt64INSTANCE.Lower(b), _uniffiStatus)
	}))
}
